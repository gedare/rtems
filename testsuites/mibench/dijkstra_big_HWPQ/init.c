...
#define NODE_TO_KV(node) (uint64_t)((((uint64_t)node->iDist)<<32UL)|(uint64_t)node)
...
extern void sparc64_hwpq_drain_queue( int qid );
...
//            rgnNodes[i].iDist = iDist + iCost;
    ...
    printf("\n");
#endif
    /* drain hwpq spill area */
    sparc64_hwpq_drain_queue(4);
  }
}
