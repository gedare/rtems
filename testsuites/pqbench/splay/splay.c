...
void splay_initialize( int size ) {
  int i;
...
void splay_insert( uint64_t kv ) {
  node *n = alloc_node();
  pq_node *pn = &n->data;
  pn->key = kv_key(kv);
  pn->val = kv_value(kv);
  n->st_node.key = pn->key;
  spenq( &n->st_node, &the_tree );
}
...
uint64_t splay_min( ) {
  ...
  stn = spdeq( &the_tree.root );

  if ( stn ) {
    stn->rightlink = the_tree.root;
    stn->leftlink = NULL;
    stn->uplink = NULL;
    if ( the_tree.root )
      the_tree.root->uplink = stn;
  }
  the_tree.root = stn;
...
uint64_t splay_pop_min( ) {
  uint64_t kv;
  node *n;
  pq_node *p;
  splay_tree_node *stn;

  stn = spdeq( &the_tree.root ); // TODO: use O(1) dequeue
