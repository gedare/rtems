...
rtems_task ST_Cache_Task(rtems_task_argument argument)
{
  int *p, x;
...
rtems_task ST_Periodic_Task(rtems_task_argument argument)
{
  rtems_id          rmid;
  rtems_status_code status;
...
rtems_task ST_Workload_Task(rtems_task_argument argument)
{
  int               index;
  rtems_status_code status;
  ...
  /* initialize ST structures */
  initialize(argument - 1);

  /* Barrier: tasks will be released by the init function */
  status = rtems_semaphore_obtain(tasks_complete_sem, RTEMS_DEFAULT_OPTIONS, 0);
  
  /* reach ST steady state */
  warmup(argument - 1);
...
)
{
  ...
  for ( index = 1 ; index <= NUM_APERIODIC_TASKS ; index++ ) {
    status = rtems_task_start( Task_id[ index ], ST_Workload_Task, index );
    directive_failed( status, "rtems_task_start loop" );
  }

  for ( index = NUM_APERIODIC_TASKS+1 ; index <= NUM_TASKS ; index++ ) {
    status = rtems_task_start( Task_id[ index ], ST_Periodic_Task, index );
    directive_failed( status, "rtems_task_start loop" );
  }
  ...
  status = rtems_task_start( cacheTask_id, ST_Cache_Task, NUM_TASKS+1 );
#endif
