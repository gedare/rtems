       ...
       bool doBFS)
{
  ...
  // set the spType to the desired sp algorithm here
  spType = SP_DIK_HWPQ;
//  spType = SP_DIK_SMARTQ;               // no need to store bucket #

  // TODO: instantiate HWPQ class.
  hwpq = new HWPQ(cNodesGiven);
}
...
bool SP::sp(Node *source, Node *sink)
{
  ...
  return (hwpq->dijkstra(source, sink, this));
}
#else
