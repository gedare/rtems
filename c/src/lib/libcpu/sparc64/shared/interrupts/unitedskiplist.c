...
uint64_t sparc64_unitedskiplist_initialize( int qid, size_t max_pq_size )
{
  int i;
  uint64_t reg = 0;
...
uint64_t sparc64_unitedskiplist_search(int queue_idx, uint64_t kv)
{
  uint64_t rv;
  uint32_t key = kv_key(kv);
...
sparc64_unitedskiplist_fill_node(
  ...
  if (exception) {
    ...
    return sparc64_unitedskiplist_handle_spill(queue_idx, count);
  }
  return 0;
}
 ...
 */
uint64_t sparc64_unitedskiplist_handle_fill(int queue_idx, uint64_t count)
{
 Chain_Control *spill_pq;
 int            i = 0;
...
uint64_t sparc64_unitedskiplist_drain( int qid, uint64_t ignored )
{
...
sparc64_spillpq_operations sparc64_unitedskiplist_ops = {
  sparc64_unitedskiplist_initialize,
  sparc64_spillpq_null_handler,
  sparc64_spillpq_null_handler,
  sparc64_spillpq_null_handler,
  sparc64_unitedskiplist_extract,
  sparc64_unitedskiplist_search,
  sparc64_unitedskiplist_handle_spill,
  sparc64_unitedskiplist_handle_fill,
  sparc64_unitedskiplist_drain
};
